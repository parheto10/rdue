...
class Espece(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None):
        ...
        super(Espece, self).save(force_insert, force_update, using)
...
class Cooperative(models.Model):
    ...
    logo = models.ImageField(upload_to='logo_coop/',null=True, blank=True)
    etat = models.BooleanField(default=True)
...
class Producteur(models.Model):
    ...
    contacts = models.CharField(max_length=50, blank=True, null=True)
    photo = models.ImageField(upload_to='photo_prod/',null=True, blank=True)
    ...
    #communaute
    groupe = models.ForeignKey(GroupeProducteur, on_delete=models.CASCADE, null=True, blank=True)
...
class Parcelle(models.Model):
    ...
    image_du_titre_de_propriete = models.ImageField(upload_to='titre_de_propriete/', null=True, blank=True)
    fichier_de_mappage = models.ImageField(upload_to='fichier_mapping/', null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class RecolteProducteur(models.Model):
    ...
    check_gap = models.BooleanField(default=True)
    gap = models.DecimalField(max_digits=1000, decimal_places=2, null=True, blank=True,default=0)
    ...
    def save(self, force_insert=False, force_update=False, using=None):
        self.estimation_production = self.parcelle.superficie * 650
        if (self.poids_total > self.estimation_production): 
            # raise ValidationError({
            #     'poids_total': f"Le poids total ({self.poids_total} kg) est supérieur à l'estimation de production ({estimation_production} kg)"
            # })
            self.gap = float(self.poids_total) - float(self.estimation_production)
