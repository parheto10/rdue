...
from .serializers import CampagneProjetSerializer, CampagneSerializer, CertificationSerializer, \
    ...
    MonitoringSerializer, ObservationMortaliteSerializer, PSESerializer, ParcelleSerializer, PlantingSerializer, ProducteurSerializer, \
...
class ProjetList(generics.ListAPIView):
    ...
    def get_queryset(self):
        if 'projID' in self.request.GET :
            ...
            # print(nb_parcelles)
            # superficie_projet  = models.Parcelle.objects.filter(producteur__section__cooperative__projet_id=int(projID)).aggregate( total=Sum('superficie'))['total']
            # print('Superficie Projet : %s' % (superficie_projet))
...
#export production Liste de Pdf with pdf
def generate_pdf_with_qrcode(request):  
    ...
    if campagne == "":
        productionListe = models.RecolteProducteur.objects.filter(parcelle__producteur__section__cooperative_id = int(coopId)).values().order_by('parcelle__producteur__nomComplet')
        print(productionListe)
    else:
        productionListe = models.RecolteProducteur.objects.filter(parcelle__producteur__section__cooperative_id = int(coopId), campagne_id=int(campagne)).values().order_by('parcelle__producteur__nomComplet')
        print(productionListe)
    
    if format_exp == "PDF":
        # Étape 1 : Récupérer les données de l'API
        apiUrl = ""
        data = JsonResponse(list(productionListe), safe=False)
        print (data)
        df = pd.DataFrame(data)
        print(data)
        print(df)
        qr_content = "http://localhost:3000/coops/productions-list/5"
        ...
        qr_image.show()
        qr_image.save("newQRCODE.png")
        buffer = BytesIO()
        template_path = 'pdf/tracability_pdf.html'
        context = {
            'cooperative':cooperative,
            'productionListe':productionListe,
            # 'elements': elements,
            'qr_image': qr_image,
        }

        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="Tracability.pdf"'
        ...
        columns = ['PRODUCTEUR', 'CODE PARCELLE','LATITUDE', 'LONGITUDE', 'SUPERFICIE',  'ESTIMATION', 'QTE LIVRE', 'GAPPE']
        ...
        font_style = xlwt.XFStyle()
        rows = productionListe.values_list(
            'parcelle__producteur__nomComplet',
            'code',
            'parcelle__latitude',
            'parcelle__longitude',
            'parcelle__superficie',
...
#Gestion paiement pour PSE
class PseList(generics.ListAPIView):
    ...
    def get_queryset(self):
        if 'coopID' and 'page' in self.request.GET:
            coopID = self.request.GET['coopID']
            limit = self.request.GET['page']
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur__section__cooperative_id = int(coopID))[:int(limit)] # super > 4 ha non mapper
            print(queryset)

        if 'prodCode' in self.request.GET:
            prodCode = self.request.GET['prodCode']
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur_id = prodCode)
        ...
        if 'coopID' in self.request.GET:
            coopID = self.request.GET['coopID']
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur__section__cooperative_id = int(coopID))

        if 'manager' in self.request.GET:
            manager = self.request.GET['manager']
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur__section__cooperative__respo = int(manager))
        if 'q' and 'co' in self.request.GET:
            q = self.request.GET['q']
            coopID = self.request.GET['co']
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur__section__cooperative_id = int(coopID)).filter(Q(planting__parcelle__producteur__nomComplet__icontains = q) | Q(planting__parcelle__code__icontains = q))

        if 'code' and 'coop' in self.request.GET:
            code = self.request.GET['code']
            coop = self.request.GET['coop']
            parcelle = models.Monitoring.objects.filter(planting__parcelle__code = code).delete()
            queryset = models.Monitoring.objects.filter(planting__parcelle__producteur__section__cooperative_id = int(coop))

        if 'sectionID' in self.request.GET:
            sectionID = self.request.GET['sectionID']
            queryset = models.Planting.objects.filter(planting__parcelle__producteur__section_id = int(sectionID))
...
def create_new_parcelle(request):
    ...
    try:
        ...
        if len(models.Parcelle.objects.filter(producteur_id = producteur.code)) >= 0 :
            tot ='-P'+str(len(models.Parcelle.objects.filter(producteur_id = producteur.code)) + 1)
...
def export_production_cooperative(request):
    format_exp = request.GET.get('format')
    campagne = request.GET.get('campagne')
    coopId = request.GET.get('coopID')
    cooperative = models.Cooperative.objects.get(id=int(coopId))
    ...
    if campagne == '':
        production = models.RecolteProducteur.objects.filter(parcelle__producteur__section__cooperative_id = int(coopId)).order_by('parcelle__producteur__nomComplet')
    else :
        production = models.RecolteProducteur.objects.filter(parcelle__producteur__section__cooperative_id = int(coopId), campagne_id=int(campagne)).order_by('parcelle__producteur__nomComplet')
    ...
    if format_exp == 'PDF' :
        ...
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="productions.pdf"'
        ...
        columns = ['PRODUCTEUR', 'PARCELLE', 'LATITUDE', 'LONGITUDE', 'SUPERFICIE', 'ESTIMATION', 'QTE LIVRE', 'GAPPE']
